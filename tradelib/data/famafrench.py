...
from tradelib.data.base import PriceDataService
from tradelib.utils.config_logger import setup_logger

logger = setup_logger(__name__)


class FamaFrench(PriceDataService):
    def __init__(self):
        super().__init__()
        self._ff_df = pd.DataFrame()
        self.latest_date = None
        self._temp_dir = "tradelib/data/temp"
    ...
    def get(
        ...
        Args:
            ...
            update: If True, downloads and updates the Fama & French data
                before returning the results. Defaults to True.
    ...
    def _download_and_prepare_data(self) -> None:
        ...
        try:
            ...
            urllib.request.urlretrieve(ff_url, f"{self._temp_dir}/fama_french.zip")
        except URLError as e:
            raise URLError(f"An error occured when downloading data: {e}")
        # open zip file in temp folder, create .csv, create pandas df, delete files
        with zipfile.ZipFile(f"{self._temp_dir}/fama_french.zip", "r") as z_file:
            z_file.extractall(f"{self._temp_dir}/")
            ff_factors = pd.read_csv(
                f"{self._temp_dir}/F-F_Research_Data_Factors_daily.CSV", skiprows=4
            )
        try:
            os.remove(f"{self._temp_dir}/fama_french.zip")
            os.remove(f"{self._temp_dir}/F-F_Research_Data_Factors_daily.CSV")
