...
from tradelib.data.base import FinancialDataService
...
class YFinance(FinancialDataService):
    def __init__(self):
        ...
        self.min_limit = 33
        # logger.info("The 'Yahoo! finance API' is intended for personal use "
    ...
    def create_tickers(self, ticker_symbols: list, session=None) -> dict:
        ...
        if non_valid_symbols := [
            symbol for symbol in ticker_symbols if not isinstance(symbol, str)
        ]:
            raise TypeError(
                "Not all symbols in `ticker_symbols` are of type"
    ...
    def ticker_data_historic(
        self, ticker_symbols: list, pct_change: bool = False, **kwargs
    ) -> pd.DataFrame:
        return self.download(ticker_symbols, pct_change, **kwargs)
    ...
    def download(
        ...
        **kwargs,
        ...
        df = yf.download(ticker_symbols, progress=progress, **kwargs)
        if pct_change:
    ...
    def symbol_to_isin(self, tickers: dict) -> dict:
        ...
        if non_valid := [
            ...
            if not isinstance(tickers[ticker], yf.Ticker)
        ]:
            raise TypeError(
                "Not all symbols in `tickers` are of type "
                " yf.Ticker; The following non valid tickers have"
                f" been provided: {non_valid}"
            )
        return {
            symb: (tickers[symb].isin if tickers[symb].isin != "-" else "")
            for symb in tickers
        }
