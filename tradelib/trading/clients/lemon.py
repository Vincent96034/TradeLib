...
class Lemon(TradeBackend):
    ...
    Args:
        config (RunConfiguration): A RunConfiguration object containing the
            required API keys and configuration details for the LemonMarkets
            account.

    Attributes:
        client (lemon.markets.api.LemonMarkets): An API client for accessing
        LemonMarkets.
    """

    def __init__(
        self, LM_data_key: str, LM_trading_key: str, LM_trading_type: str
    ) -> None:
        raise DeprecationWarning(
            "This class is deprecated. Please use another trading backend."
        )
    ...
    def get_bank_statements(
        self, type_: Optional[str] = None, from_: Optional[str] = "beginning"
    ) -> list:
        ...
        Args:
            type_ (Optional[str]): Type of bank statements. Defaults to None.
                Can be one of the following:
                    ...
                    - "dividend"
                    - "tax_refunded"
            from_ (Optional[str]): Filter for bank statements after a specific
                date. Format: "YYYY-MM-DD". Defaults to "beginning".

        Returns:
            list: A list of BankStatement objects.
        """
        response = self.client.trading.account.get_bank_statements(
            type=type_, from_=from_
        )
    ...
    def get_performance(
        ...
        Args:
            ...
            from_ (Optional[str]): The starting date (ISO format: "YYYY-MM-DD")
                for the performance data. Defaults to None.
            to (Optional[str]): The ending date (ISO format: "YYYY-MM-DD") for
                the performance data. Defaults to None.
        ...
        response = self.client.trading.positions.get_performance(
            isin=isin, from_=from_, to=to, sorting=sorting
        )
        return response.results
    ...
    def place_order(
        ...
        **kwargs,
        ...
        """Places a trading order for a given ISIN with the specified side and
        quantity.
        ...
        Args:
            ...
            quantity_type (str): The type of quantity, either "amount"
                (number of stocks) or "value".
            activate (bool, optional): Whether to activate the order
                immediately. Defaults to False.
            **kwargs: Additional keyword arguments to pass to the order
                creation method.

        Returns:
            dict: A dictionary containing the order ID, order object, and
                whether the order was activated.

        Warnings:
            If the requested order quantity exceeds the available quantity of a
                security in the portfolio, the function will adjust the order
                quantity to the maximum available amount and log a warning.
            If the requested order quantity is less than or equal to zero, the
                function will dismiss the order and log a warning.

        Notes:
            If the `quantity_type` argument is "value", the function will
            convert the quantity from a monetary value to a number of stocks
            using the `value_to_amount` method.
        """
        # input validation
        ...
        if order_amount <= 0:
            logger.warning(f"""Order quantity {order_amount} of {isin} is less
                than or equal to zero. Order will be dismissed.""")
            return
    ...
    def get_order(self, order_id: str) -> list:
    ...
    def place_multi_order(
        ...
        Args:
            order_list (list): A list of dictionaries, where each dictionary
                represents an order. Each order dictionary must contain the
                following keys: 'isin' (str), 'side' (str), 'quantity' (float),
                'quantity_type' (str).
            activate (bool, optional): Whether to activate the orders
                immediately after placing them. Defaults to True.
        ...
        Raises:
            ValueError: If any of the orders in the list is missing a
                required key.
        """
        # Input validation
        order_responses = []
    ...
    def get_latest_data(self, isin: str, **kwargs):
        ...
        Args:
            ...
            **kwargs: Additional optional parameters:
                - mic (str): The market identifier code to retrieve the market
                    data for.
                - decimals (bool): Whether to return decimal values in the
                    response.
                - epoch (bool): Whether to return timestamps in epoch format in
                    the response.
        ...
        Returns:
            If the response contains a single result, returns a dictionary with
                the market data for the specified ISIN.
            If the response contains multiple results, returns a list of
                dictionaries, each containing the market data for a different
                timestamp.
        """
    ...
    def value_to_amount(self, isin: str, side: str, value: Union[int, float]) -> int:
        ...
        Args:
            ...
            value (Union[int, float]): The value to convert to the
                corresponding amount of stocks.
        ...
        return int(
            Decimal(value / self.convert_to_hcents(price)).quantize(0, ROUND_HALF_UP)
        )

    @staticmethod
    def convert_to_hcents(amount: float) -> int:
        """Converts a currency amount to an integer value in hundreths of a
        cent. Example: 123.45 -> 1234500
