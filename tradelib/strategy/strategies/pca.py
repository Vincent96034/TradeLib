...
from tradelib.data.yfinance import YFinance
from tradelib.data.famafrench import FamaFrench
...
class PCAStrategy(Strategy):
    ...
    Args:
    ratio (float): The ratio of companies to select for the portfolio based
        on their sensitivity to the first principal component.
    ...
    portfolio_type (str): The type of portfolio to construct. Can be either
        'head', 'tail' or 'hedge'. Default is 'tail'.
    factor_estimate_cov (bool): Whether to estimate covariance matrix based
        on Fama-French factors. Default is True.
    ...
    loadings (list): The loadings on the first principal component of the
        returns data.
    sign_ratio (float): The ratio of companies with negative loadings to
        those with positive loadings.
    ...
    _df_factors (pd.DataFrame): The Fama-French factors data used to
    ...
    run_strategy: Runs the PCA-based trading strategy and returns the
        weights assigned to each company in the portfolio.
    loadings_to_weights: Converts the loadings on the first principal
        component of the returns data to weights assigned to each company
        in the portfolio.
    estimate_cov_with_factors: Estimates the covariance matrix of the
        returns data using Fama-French factors.
    """
    ...
    def _get_returns_data(
        ...
        df_returns = self.data_service.get(
            companies, start=start_date, end=stop_date, pct_change=True
        )[OHLC_spec]
        self._df_returns = df_returns
        return df_returns.dropna()

    def _get_factor_data(self, start_date):
        df_factors = self.famafrench_data.get(start_date)
        self._df_factors = df_factors
        return df_factors, self.famafrench_data.latest_date

    def generate_signals(self, train_period: int = 24):
        """Run PCA-Strategy."""
        start_date = (dt.today() - relativedelta(months=train_period)).strftime(
            "%Y-%m-%d"
        )
        ...
        n = math.ceil(len(returns_df.columns) * self.ratio)
        weights, sign_ratio = self.loadings_to_weights(
            loadings=loadings,
    ...
    @staticmethod
    def loadings_to_weights(
        loadings: list, col_names: list, portfolio_type: str, n: int
    ):
    ...
    def estimate_cov_with_factors(X, F, K):
        """Estimates the covariance matrix based on the Farma & French Factor
        Model.
    ...
    def ratio(self, value):
        ...
        if (value <= 0) or (value > 1):
            raise ValueError(
                "`ratio` must be greater than 0 and smaller or equal to 1."
            )
        self._ratio = value
    ...
    def company_pool(self, value):
        if isinstance(value, list):
            if non_valid_symbols := [
                symbol for symbol in value if not isinstance(symbol, str)
            ]:
                raise TypeError(
                    "Not all symbols in `ticker_symbols` are of type string. The"
            ...
            if value not in self._valid_inputs["company_pool"]:
                raise ValueError(
                    f"`company_pool` must be one of {self._valid_inputs['company_pool']}"
    ...
    def portfolio_type(self, value):
        ...
        if value not in self._valid_inputs["portfolio_type"]:
            raise ValueError(
                f"`portfolio_type` must be one of {self._valid_inputs['portfolio_type']}."
                f" Received {value}"
            )
