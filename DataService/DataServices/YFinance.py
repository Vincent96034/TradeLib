...
class YFinance(FinancialDataService):
    def __init__(self):
        ...
        self.min_limit = 33
        logger.warn("The 'Yahoo! finance API' is intended for personal use only. You should"\
    ...
    def create_tickers(self, ticker_symbols: list, session = None) -> dict:
        """ Creates a dictionary of yf.Ticker objects for the given list of ticker symbols.

            Args:
                ticker_symbols (list): A list of ticker symbols as strings.
                session (requests.Session, optional): A requests.Session object to use for the HTTP
                    requests. Defaults to None.

            Returns:
                dict: A dictionary of Ticker objects, where the keys are the ticker symbols and 
                    the values are Ticker objects.
        """
        ...
        try:
            ...
            request = {}
            logger.warn("Encountered TimeoutError, retrying now ...")
    ...
    def ticker_data_historic(self, ticker_symbols: list, pct_change: bool = False, **kwargs) -> pd.DataFrame:
        """ Returns historic data for the given ticker symbols.
            ...
            Keyword Args:
                start (str or None): The start date for the data in the format "YYYY-MM-DD".
                end (str or None): The end date for the data in the format "YYYY-MM-DD".
                interval (str or None): The frequency of the data, either "1d" (daily) or "1wk" (weekly).
                group_by (str or None): The grouping of the data, either "ticker" or "column".
                [...]

            Returns:
                pandas.DataFrame: A pandas DataFrame with the historic data for the given ticker symbols.
                    The DataFrame has the following columns for each ticker-symbol:
                        - `Open`: the opening price for each trading day.
                        - `High`: the highest price reached during each trading day.
                        - `Low`: the lowest price reached during each trading day.
                        - `Close`: the closing price for each trading day.
                        - `Adj Close`: the adjusted closing price for each trading day.
        ...
        if not isinstance(kwargs.get('start'), (str, type(None))):
            raise TypeError("`start` must be of type string in the format YYYY-MM-DD.")
    ...
    def symbol_to_isin(self, tickers: dict) -> dict:
        """ Returns a dictionary of ISIN codes for the given ticker symbols.
            ...
            Args:
                tickers (dict): A dictionary of ticker symbols and yf.Ticker objects.

            Returns:
                dict: A dictionary of ISIN codes, where the keys are the ticker symbols and the 
                    values are the corresponding ISIN codes as strings. If the ISIN code is not 
                    available, the corresponding value is an empty string.
        """
