...
class Lemon(TradeBackend):
    ...
    def place_order(self,
        ...
        if quantity_type not in ["amount", "value"]:
            raise ValueError(
                "'quantity_type' must be either 'amount' or 'value'.")

        if quantity_type == "amount":
            order_amount = quantity
        else:  # transform value to number of stocks
            order_amount = self.value_to_amount(
                isin=isin, side=side, value=quantity)
        ...
        if activate and (order.results.status == "inactive"):
            activate_order = self.client.trading.orders.activate(
                order_id=order_id)
        logger.info(
            f"Order {order_id} placed{' and activated' if activate else ''}.")
        return {
            "order_id": order_id,
            "order_obj": order,
